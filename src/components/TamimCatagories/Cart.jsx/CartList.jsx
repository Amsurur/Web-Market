...
const CartList = () => {
  ...
  console.log(cartObj);
  const productId = useSelector((store) => store.onlineShop.productId);
  ...
  const [products, setProducts] = useState(cartObj);
  // console
  //   .log
  //   products.map((e) => {
  //     return e.price;
        ...
        {products.map((product, index) => (
          <React.Fragment key={index}>
            <div className="flex gap-[25px] items-center max:flex-wrap dark:text-white">
              <img
                src={"http://localhost:3000/" + cartObj[index]?.media[0]?.src}
                alt=""
                className="self-start lg:w-[30%] md:w-[35%] sm:w-[40%] w-[100%] object-cover"
              />
              <h1>{product.name}</h1>
            </div>
